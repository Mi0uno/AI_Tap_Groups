...
**AI-powered tab grouping for Google Chrome and Microsoft Edge**

_用你配置的 AI 模型，自动整理 Chrome / Edge 标签页分组。_

![Manifest V3](https://img.shields.io/badge/Manifest-V3-blue?style=flat-square)
![Google Chrome](https://img.shields.io/badge/Google%20Chrome-120%2B-4285F4?style=flat-square)
...
**AI Tab Grouper** 是一个 Chrome / Edge Manifest V3 扩展。它读取当前浏览器窗口或全部窗口的标签页标题、域名和 URL 摘要，调用 OpenAI 兼容的 Chat Completions 接口，让 AI 判断哪些标签页应该放到同一个浏览器原生标签组里。
...
| 项目效果                                                                       | 扩展配置与分组状态                                                                           |
| ------------------------------------------------------------------------------ | -------------------------------------------------------------------------------------------- |
| [![GitHub 项目页](docs/screenshots/demo.png)](docs/screenshots/demo.png) | [![扩展配置与分组状态](docs/screenshots/popup-settings.png)](docs/screenshots/popup-settings.png) |
...
| 项目     | 要求                                 |
| -------- | ------------------------------------ |
| 浏览器   | Google Chrome 120+ 或 Microsoft Edge，开启扩展开发人员模式 |
...
```text
E:\A_Develop\browser_plugin\AI_Tap_Groups
```
...
> 修改代码后，需要回到 `chrome://extensions/` 或 `edge://extensions/` 刷新扩展，新的 background service worker 才会生效。

## 配置说明

| 配置项           | 默认值                                         | 说明                                                                 |
| ---------------- | ---------------------------------------------- | -------------------------------------------------------------------- |
| AI 提供商        | `OpenAI`                                      | 内置 OpenAI、Anthropic、Gemini、DeepSeek、Xiaomi MiMo、MiniMax、GLM、OpenRouter 免费模型和自定义。 |
...
选择提供商后，扩展会自动填入接口地址、默认模型和鉴权方式。用户只需要输入对应平台的 API Key。

| 提供商 | 默认模型 | 接口类型 | 默认接口 | 备注 |
| --- | --- | --- | --- | --- |
| OpenAI | `gpt-4.1-mini` | Chat Completions | `https://api.openai.com/v1/chat/completions` | 官方 OpenAI API。 |
| Anthropic Claude | `claude-sonnet-4-20250514` | Messages API | `https://api.anthropic.com/v1/messages` | 使用 `x-api-key` 和 `anthropic-version`。 |
| Google Gemini | `gemini-2.5-flash` | OpenAI Compatibility | `https://generativelanguage.googleapis.com/v1beta/openai/chat/completions` | Google AI Studio 通常提供免费层。 |
| DeepSeek | `deepseek-v4-flash` | Chat Completions | `https://api.deepseek.com/chat/completions` | DeepSeek V4 请求会自动关闭 thinking。 |
| Xiaomi MiMo | `mimo-v2.5` | OpenAI Compatibility | `https://api.xiaomimimo.com/v1/chat/completions` | 使用 `api-key` 请求头。 |
| MiniMax | `MiniMax-M2.7-highspeed` | OpenAI Compatibility | `https://minimax-m2.com/api/v1/chat/completions` | 使用 OpenAI 兼容格式。 |
| Z.ai / GLM | `glm-5.1` | Chat Completions | `https://api.z.ai/api/paas/v4/chat/completions` | GLM 系列 API。 |
| OpenRouter 免费模型 | `openrouter/free` | OpenAI Compatibility | `https://openrouter.ai/api/v1/chat/completions` | 路由到 OpenRouter 当前可用免费模型，具体可用性以平台为准。 |
| 自定义 | `your-model-id` | OpenAI Compatibility | 自行填写 | 保留自定义网关、代理和私有部署能力。 |

### 免费或免费额度模型

目前适合内置的免费/免费额度入口：

- **Google Gemini Developer API**：Google AI Studio / Gemini API 提供免费层，适合轻量标签分类。
- **OpenRouter 免费模型**：OpenRouter 会列出当前可用的 `:free` 或免费路由模型，可用模型会变化，扩展保留为一个独立预设。

其他厂商常见的是注册送额度、限时体验或地区/账号条件免费，不适合硬编码为稳定免费模型。你仍然可以通过“自定义（OpenAI 兼容）”接入这些服务。
...
| `tabGroups`                    | 创建、更新和合并 Chrome / Edge 原生标签组。               |
...
| `src/providers.js`  | AI 提供商预设、默认模型、鉴权方式和接口地址。     |
...
Chrome Web Store 或手动分发可直接打包源码目录。建议排除 `.git/`，保留：

```text
manifest.json
icons/
src/             # 包含 providers.js、background.js、popup.*
docs/            # 可选，仅 README 截图资源
README.md        # 可选，商店包可不包含
```

调试建议：

- 修改代码后刷新 `chrome://extensions/` 或 `edge://extensions/` 中的扩展。
...
扩展使用 Manifest V3 service worker、`tabGroups` 和 `alarms`。`tabGroups` 本身在 Chrome 89+ 可用，但 Chrome 120 起 `alarms` 支持 30 秒最小周期，更适合本项目的秒级 debounce + 后台兜底模型。因此 manifest 声明了 `minimum_chrome_version: "120"`。
